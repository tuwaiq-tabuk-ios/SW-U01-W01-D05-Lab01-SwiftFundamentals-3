 ...
 */
func spaceAvailableMessage(eachVideoDuration: Int,
                           numberOfVideos: Int) -> String {
  let currentSpace = 10000
  let megabytesPerVideoSecond = 3
  let spaceAvailable =
  currentSpace - eachVideoDuration * numberOfVideos * megabytesPerVideoSecond
...
let videoMessage = spaceAvailableMessage(eachVideoDuration: desiredVideoDuration,
...
func sum(number1:Int, number2: Int) -> String{
  let sum = number1 + number2
  return "result is \(sum)"
}

let resultOfSum = sum(number1: 5, number2: 6)
print(resultOfSum)
